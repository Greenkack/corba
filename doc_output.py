...
def render_pdf_ui(
    ...
    with st.expander(" VISUALISIERUNGEN & DIAGRAMME", expanded=True):
    ...
    with st.expander(" DRAG & DROP BEARBEITUNG (EXPERIMENTAL)", expanded=False):
        enable_dragdrop = st.checkbox(" Drag & Drop Bearbeitung aktivieren", value=False, key="pdf_enable_dragdrop")
    ...
    # === DIAGRAMM-OPTIONEN ===
    st.markdown("---")
    with st.expander(" Diagramm-Optionen", expanded=True):
    ...
    with st.expander(" ERWEITERTE PDF-EINSTELLUNGEN", expanded=False):
        ...
        with col1:
            ...
            enable_dragdrop = st.checkbox(" PDF Drag & Drop aktivieren", value=False)
