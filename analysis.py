...
def format_kpi_value(
    ...
    if isinstance(value, (int, float)):
        ...
        if unit == "Jahre":
            return get_text(
                current_texts, "years_format_string", "{val:.1f} Jahre"
            ).format(val=value)
...
def _render_overview_section(results: Dict[str, Any], texts: Dict[str, str], viz_settings: Dict[str, Any]):
    ...
    • Gesamtverbrauch: {annual_consumption_total:.0f} kWh
    ...
    with col2:
        ...
        if selected_color_method == "Palette":
            ...
            new_secondary_color = st.color_picker(
                ...
                value=st.session_state.get(key_secondaryColor),
...
def render_feed_in_revenue_switcher(
    ...
    fig = create_universal_2d_chart(
        ...
        title=get_text(
            ...
            f"Monatliche Einspeisevergütung (Tarif: {feed_in_tariff_eur_kwh*100:.1f} ct/kWh)",
        ),
...
def render_tariff_cube_switcher(
    ...
    title = get_text(
        ...
        f"Tarifvergleich bei {jahresverbrauch:.0f} kWh/Jahr",
    )
    ...
    if fig:
        analysis_results["tariff_cube_switcher_chart_bytes"] = (
            _export_plotly_fig_to_bytes(fig, texts)
        )
    else:
...
def render_extended_calculations_dashboard(
    ...
    with tabs[1]:  # Netzanalyse
        ...
        with col1:
            st.metric(
                "Netzeinspeisung", f"{grid_results['grid_feed_in_kwh']:,.0f} kWh/Jahr"
            )
            st.metric("Netzbezug", f"{grid_results['grid_purchase_kwh']:,.0f} kWh/Jahr")
    ...
    with tabs[5]:  # Umwelt
        ...
        with col1:
            st.metric(
                "CO₂-Einsparung/Jahr", f"{env_results['annual_co2_savings_tons']:.1f} t"
            )

        with col2:
            st.metric(
                "Wie viele Bäume", f"{env_results['trees_equivalent']:,.0f} Bäume"
            )

        with col3:
            st.metric(
                "Auto-km Äquivalent", f"{env_results['car_km_equivalent']:,.0f} km"
            )

        with col4:
            st.metric("Wasser gespart", f"{env_results['water_savings_liters']:,.0f} L")
...
def render_investment_value_switcher(
    ...
    analysis_results["investment_value_switcher_chart_bytes"] = (
        _export_plotly_fig_to_bytes(fig, texts)
    )
...
def render_storage_effect_switcher(
    ...
    analysis_results["storage_effect_switcher_chart_bytes"] = (
        _export_plotly_fig_to_bytes(fig, texts)
    )
...
def render_cost_growth_switcher(
    ...
    for s_percent in szenarien_prozent_vals:
        ...
        chart_data["series"].append(
            {"name": f"{s_percent:.1f}% p.a.", "data": kosten_kwh_pro_jahr}
        )
    ...
    if selected_chart_type == "bar":
        fig.add_trace(
            go.Bar(
                ...
                text=[f"{x:.0f}" for x in monthly_prod],
                texttemplate="%{text}",
        ...
        fig.add_trace(
            go.Bar(
                ...
                text=[f"{x:.0f}" for x in monthly_cons],
                texttemplate="%{text}",
                textposition="outside",
            )
        )
...
def _create_electricity_cost_projection_chart(
    ...
    ).format(steigerung=f"{price_increase_proj:.1f}")
...
def render_analysis(
    ...
    try:
        ...
        st.warning("⚠️ Erweiterte Live-Vorschau nicht verfügbar")
    ...
    try:
        ...
        st.warning("⚠️ Moderne Diagramme nicht verfügbar - verwende Standard-Charts")
            ...
            if pdf_data:
                ...
                if extended_summary:
                    ...
                    with col1:
                        ...
                        st.metric(
                            "IRR",
                            f"{extended_summary.get('irr_internal_rate_of_return', 0):.1f}%",
                        )
                        st.metric(
                            "CO₂-Einsparung",
                            f"{extended_summary.get('co2_gesamteinsparung_tonnen', 0):.1f} t",
                        )

                    with col2:
                        st.metric(
                            "CO₂-Amortisation",
                            f"{extended_summary.get('co2_amortisationszeit_jahre', 0):.1f} Jahre",
                        )
                        st.metric(
                            "Verschattungsverlust",
                            f"{extended_summary.get('verschattungsverlust_prozent', 0):.1f}%",
                        )
                        st.metric(
                            "Temperaturverlust",
                            f"{extended_summary.get('temperaturverlust_prozent', 0):.1f}%",
                        )

                    with col3:
                        st.metric(
                            "WR-Wirkungsgrad",
                            f"{extended_summary.get('wechselrichter_wirkungsgrad', 0):.1f}%",
                        )
                        st.metric(
                            "Recycling-Quote",
                            f"{extended_summary.get('recycling_quote_prozent', 0):.0f}%",
                        )
                        st.metric(
                            "Top-Optimierung",
                            f"{extended_summary.get('top_optimierung_nutzen', 0):.0f} €/Jahr",
                        )
...
def render_advanced_financial_analysis(results: Dict[str, Any], texts: Dict[str, str]):
    ...
    if investment > 0 and annual_savings > 0:
        ...
        with col1:
            ...
            st.metric("📈 ROI (20 Jahre)", f"{roi_20_years:.1f}%")
        ...
        with col2:
            ...
            st.metric("💎 NPV (20 Jahre)", f"{npv:,.0f}€")
        
        with col3:
            # IRR Schätzung (vereinfacht)
            irr_estimate = (annual_savings / investment) * 100
            st.metric("📊 IRR (geschätzt)", f"{irr_estimate:.1f}%")
...
def render_advanced_energy_analysis(results: Dict[str, Any], texts: Dict[str, str]):
    ...
    if annual_production > 0 and annual_consumption > 0:
        ...
        with col4:
            ...
            st.metric("🔋 Kapazitätsfaktor", f"{capacity_factor:.1f}%")
...
def render_advanced_environmental_analysis(results: Dict[str, Any], texts: Dict[str, str]):
    ...
    if co2_annual > 0:
        ...
        with col1:
            st.metric("🌿 CO₂-Einsparung/Jahr", f"{co2_annual/1000:.1f} t")
        
        with col2:
            co2_20_years = co2_annual * 20 / 1000
            st.metric("🌍 CO₂-Einsparung (20J)", f"{co2_20_years:.1f} t")
...
def render_advanced_technical_analysis(results: Dict[str, Any], texts: Dict[str, str]):
    ...
    with col1:
        ...
        system_data = {
            "Anlagengröße": f"{results.get('anlage_kwp', 0.0):.1f} kWp",
            "Module Anzahl": f"{results.get('module_quantity', 0):,}",
            "Modulleistung": f"{results.get('module_power_wp', 0)} Wp",
            "Wechselrichter-Effizienz": f"{results.get('inverter_efficiency_percent', 0.0):.1f}%",
            "Systemverluste": f"{results.get('system_losses_percent', 0.0):.1f}%",
            "Performance Ratio": f"{results.get('performance_ratio_percent', 0.0):.1f}%"
        }
    ...
    with col2:
        ...
        energy_data = {
            "Jahresproduktion": f"{results.get('annual_pv_production_kwh', 0.0):,.0f} kWh",
            "Eigenverbrauch": f"{results.get('eigenverbrauch_pro_jahr_kwh', 0.0):,.0f} kWh",
            "Netzeinspeisung": f"{results.get('netzeinspeisung_kwh', 0.0):,.0f} kWh",
            "Netzbezug": f"{results.get('grid_bezug_kwh', 0.0):,.0f} kWh",
            "Autarkiegrad": f"{results.get('self_supply_rate_percent', 0.0):.1f}%",
            "Batteriekapazität": f"{results.get('battery_capacity_kwh', 0.0):.1f} kWh"
        }
    ...
    if anlage_kwp > 0:
        ...
        degradation_data = {
            ...
            "Verlust (%)": [f"{(1 - p/anlage_kwp)*100:.1f}" for p in power_values]
        }
    ...
    if base_savings > 0:
        ...
        for name, data in scenarios.items():
            ...
            scenario_data.append({
                "Szenario": name,
                "Jährliche Ersparnis": f"{savings_scenario:,.0f}€",
                "Amortisationszeit": f"{payback_scenario:.1f} Jahre",
                "20-Jahres-Ersparnis": f"{savings_scenario * 20:,.0f}€",
