...
def perform_calculations(
    ...
    if price_matrix_excel_bytes and isinstance(price_matrix_excel_bytes, bytes):
        ...
        price_matrix_df_for_lookup = parse_module_price_matrix_excel(
        ...
        if (
            ...
            price_matrix_df_for_lookup = (
                None  # Explizit zurücksetzen, falls Excel-Versuch fehlschlägt
            )

    if (
        price_matrix_df_for_lookup is None
    ):  # Nur wenn Excel nicht erfolgreich war oder nicht vorhanden
        if (
            price_matrix_csv_content
            and isinstance(price_matrix_csv_content, str)
            and price_matrix_csv_content.strip()
        ):
            ...
            price_matrix_df_for_lookup = parse_module_price_matrix_csv(
            ...
            if (
                ...
                price_matrix_df_for_lookup = (
                    None  # Sicherstellen, dass es None ist bei Fehlschlag
                )
    ...
    results["price_matrix_loaded_successfully"] = (
